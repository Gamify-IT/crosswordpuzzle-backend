...
public class CrosswordGenerator {
    ...
    private boolean tryPlaceVertical(final Intersection intersection, final String word, final Crossword crossword) {
        ...
        if (
            ...
            !(
                crossword
                    .getTile(intersection.getX(), intersection.getY() - intersection.getPositionInWord() - 1)
                    .equals(EMPTY)
            )
        ) {
            return false;
    ...
    private boolean tryPlaceHorizontal(final Intersection intersection, final String word, final Crossword crossword) {
        ...
        if (
            ...
            !(
                crossword
                    .getTile(intersection.getX() - intersection.getPositionInWord() - 1, intersection.getY())
                    .equals(EMPTY)
            )
        ) {
            return false;
        }
        crossword.placeWordHorizontal(
            intersection.getX() - intersection.getPositionInWord(),
            intersection.getY(),
            word
        );
        return true;
