...
class ConfigControllerTest {
    ...
    void getSpecificConfigurations() throws Exception {
        final MvcResult result = mockMvc
            .perform(
                get(API_URL + "/" + initialConfigDTO.getId()).cookie(cookie).contentType(MediaType.APPLICATION_JSON)
            )
            .andExpect(status().isOk())
            .andReturn();
    ...
    void testDeleteQuestions() throws Exception {
        ...
        final MvcResult result = mockMvc
            .perform(
                delete(API_URL + "/" + initialConfigDTO.getId() + "/questions/" + question.get().getId()).cookie(cookie)
            )
            .andExpect(status().isOk())
            .andReturn();
    ...
    void testDeleteQuestions_DoesNotExist_ThrowsNotFoundException() throws Exception {
        ...
        mockMvc
            .perform(
                delete(API_URL + "/" + initialConfigDTO.getId() + "/questions/" + UUID.randomUUID()).cookie(cookie)
            )
            .andExpect(status().isNotFound());
    }
    ...
    void testWithoutCookie_ThrowsBadRequest() throws Exception {
        ...
        mockMvc
            .perform(
                post(API_URL + "/" + initialConfig.getId() + "/questions/").contentType(MediaType.APPLICATION_JSON)
            )
            .andExpect(status().isBadRequest());
        mockMvc
